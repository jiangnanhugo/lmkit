...
class RNNLM(object):
    def __init__(self,n_input,n_hidden,n_output,cell='gru',optimizer='sgd',p=0.5):
        ...
        self.y_adj=T.itensor3('batched_choice_y')
    ...
    def build(self):
        ...
        output_layer=level_softmax(softmax_shape,
                                   hidden_layer.activation,
                                   self.y_node,self.y_adj,self.y_mask)
        ...
        self.train=theano.function(inputs=[self.x,self.x_mask,self.y_node,self.y_choice,self.y_mask,self.n_batch,lr],
                                   outputs=cost,
                                   updates=updates,
