...
argument.add_argument('--prefix_filepath',default='../data/wikitext-2/wiki.train-c200-p1.out',type=str,help='classes and words prefix configures.')
...
argument.add_argument('--rnn_cell', default='fastlstm', type=str, help='recurrent unit type')
...
prefix_filepath=args.prefix_filepath
...
def train(lr):
    ...
    train_data=TextIterator(train_datafile,prefix_filepath=prefix_filepath,n_batch=n_batch)
    ...
    for epoch in xrange(NEPOCH):
        error=0
        for x,x_mask,(y_node,y_arange_cache),y_mask in train_data:
            idx+=1
            cost=model.train(x,x_mask,y_node,y_arange_cache,y_mask,lr)
            error+=cost
