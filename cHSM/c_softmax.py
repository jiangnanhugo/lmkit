...
def log_softmax_masked(x, mask):
    ...
    return e_x#e_x / e_x.sum(axis=1, keepdims=True)


# class-based hierarchical softmax
class C_softmax(object):
    def __init__(self, shape, x, y_node, y_node_mask):
        """
        ...
        init_wp_matrix = np.asarray(self.rng.uniform(low=-np.sqrt(6. / (self.in_size)),
                                                     high=np.sqrt(6. / (self.in_size)),
                                                     size=(self.class_size, self.out_size, self.in_size)),
        ...
        # bias for word layer
        init_wb = np.zeros((self.class_size, self.out_size), dtype=theano.config.floatX)
